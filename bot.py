...
class PrimeScalpBot:
    ...
    def fetch_indicators(self, symbol, timeframe='5m', limit=100):
        try:
            ...
            if PANDAS_TA_AVAILABLE:
                df['tema'] = ta.tema(df['close'], length=9).fillna(0)
                df['vwap'] = ta.vwap(df['high'], df['low'], df['close'], df['volume']).fillna(0)
                st = ta.supertrend(df['high'], df['low'], df['close'], length=10, multiplier=3)
                if st is not None and not st.empty:
                    df['st'] = st.iloc[:, 0].fillna(1)
                else:
                    df['st'] = 1
                df['rsi'] = ta.rsi(df['close'], length=7).fillna(50)
                adx_res = ta.adx(df['high'], df['low'], df['close'], length=20)
                if adx_res is not None and not adx_res.empty:
                    df['adx'] = adx_res.iloc[:, 0].fillna(25)
                else:
                    df['adx'] = 25
                df['cmf'] = ta.cmf(df['high'], df['low'], df['close'], df['volume'], length=10).fillna(0)
                ...
                df['adx'] = 25
                df['cmf'] = 0.1
                df['atr'] = (df['high'] - df['low']).rolling(14).mean().fillna(1)
            return df
        except Exception as e:
    ...
    def check_entry(self, df):
        ...
        tema = float(last.get('tema', 0))
        vwap = float(last.get('vwap', 0))
        st = float(last.get('st', 1))
